...
def fetch_ticker_batch(instrument_names):
    ticker_data = {}
    try:
        params = {"instrument_name": ",".join(instrument_names)}
...
def fetch_data(instruments_tuple, historical_lookback_days=7):
    ...
    dfs = []
    with ThreadPoolExecutor(max_workers=10) as executor:
        future_to_name = {executor.submit(lambda x: fetch_single_instrument_data(x, historical_lookback_days), name): name for name in instr}
    ...
    return dfc.dropna(subset=['k', 'option_type', 'mark_price_close', 'iv_close', 'expiry_datetime_col']).sort_values('date_time')
    ...
    try:
        ...
        if hist:
            data = [{'date_time': pd.to_datetime(e['timestamp'], unit='ms', utc=True),
                     'funding_rate': e['fundingRate'] * 365 * 3} for e in hist]
...
def compute_greeks_vectorized(df, spot_price, snapshot_time_utc, risk_free_rate=0.0):
    ...
    T = np.where(T < 1e-9, 1e-9, T)  # Avoid division by zero
    ...
    delta = np.where(option_type == 'C', norm.cdf(d1), norm.cdf(d1) - 1.0)
    gamma = norm.pdf(d1) / (S * sigma_sqrt_T)
    vega = S * norm.pdf(d1) * np.sqrt(T) * 0.01
    theta = np.where(option_type == 'C',
...
def plot_delta_balance(ticker_list, spot_price):
    if not ticker_list or pd.isna(spot_price):
        ...
        return
    calls_delta = np.sum([i["delta"] * i["open_interest"] for i in ticker_list if i["option_type"] == "C" and pd.notna(i["delta"]) and pd.notna(i["open_interest"])])
    puts_delta = np.sum([i["delta"] * i["open_interest"] for i in ticker_list if i["option_type"] == "P" and pd.notna(i["delta"]) and pd.notna(i["open_interest"])])
...
def main():
    ...
    if valid_expiries:
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_expiry = {executor.submit(lambda x: (x, sum(fetch_ticker(instr).get('open_interest', 0) for instr in get_option_instruments(all_instruments_list, "C", x.strftime("%d%b%y").upper(), coin)[:50] + get_option_instruments(all_instruments_list, "P", x.strftime("%d%b%y").upper(), coin)[:50])), exp): exp for exp in valid_expiries}
            expiry_oi_map = {future.result()[0]: future.result()[1] for future in as_completed(future_to_expiry)}
        if expiry_oi_map:
            best_expiry_by_oi = max(expiry_oi_map, key=expiry_oi_map.get)
            default_exp_idx = valid_expiries.index(best_expiry_by_oi) if best_expiry_by_oi in valid_expiries else 0
    ...
    sel_call_pair_default_idx = min(len(all_calls_expiry)-1, int(len(all_calls_expiry)*0.7)) if all_calls_expiry else 0
    sel_call_pair = st.sidebar.selectbox("OTM Call:", sorted(all_calls_expiry, key=lambda x: int(x.split('-')[2])), index=sel_call_pair_default_idx, key=f"main_pair_call_adv_vFull2_final_{e_str}_{coin}", disabled=not all_calls_expiry)
    sel_put_pair_default_idx = max(0, int(len(all_puts_expiry)*0.3)-1) if all_puts_expiry else 0
    sel_put_pair = st.sidebar.selectbox("OTM Put:", sorted(all_puts_expiry, key=lambda x: int(x.split('-')[2])), index=sel_put_pair_default_idx, key=f"main_pair_put_adv_vFull2_final_{e_str}_{coin}", disabled=not all_puts_expiry)
    ...
    dft_raw = fetch_data(tuple(all_instr_selected_expiry), historical_lookback_days=pair_sim_lookback_days)
    ticker_data = fetch_ticker_batch(all_instr_selected_expiry)
    valid_tickers = {k for k, v in ticker_data.items() if v and isinstance(v.get('iv'), (int, float)) and v.get('iv', 0) > 1e-4 and pd.notna(v.get('open_interest'))}
    ...
    if not dft.empty and pd.notna(spot_price):
        ...
        if not dft_latest.empty:
            dft_latest['open_interest'] = dft_latest['instrument_name'].map(lambda x: ticker_data.get(x, {}).get('open_interest', 0.0))
    ...
    if not dft_latest.empty:
        ticker_list_latest_snap = [{'instrument': row['instrument_name'], 'strike': int(row['k']), 'option_type': row['option_type'], 'open_interest': float(row['open_interest']), 'delta': float(row['delta']), 'gamma': float(row['gamma']), 'iv': float(row['iv_close'])} for _, row in dft_latest.iterrows()]
    ...
    if show_std_dh_sims:
        if not dft_with_hist_greeks.empty and not df_krak_5m.empty and pd.notna(spot_price):
            ...
            with Pool(processes=4) as pool:
                delta_df_std, hedge_actions_std = pool.apply(hedge_instance_std.run_loop, (min(7, pair_sim_lookback_days),))
            ...
            with Pool(processes=4) as pool:
                state_df_mat_std, actions_df_mat_std = pool.apply(matrix_instance_std.run_loop, (min(7, pair_sim_lookback_days),))
    ...
    if st.sidebar.checkbox("Show Delta Hedging M2M Dashboard (Dark Style)", True, key="adv_show_dh_m2m_dark_dashboard_main_vFull3_final"):
        if not delta_df_std.empty and pd.notna(spot_price):
    ...
    if not dft_with_hist_greeks.empty and not df_krak_5m.empty and pd.notna(spot_price) and e_str:
    ...
    if not dft_latest.empty:
        ...
        net_d_mm = -(dft_latest['delta'] * dft_latest['open_interest']).sum()
        cols_greeks_adv[0].metric("Net Delta", f"{net_d_mm:,.2f}")
        net_g_mm = -(dft_latest['gamma'] * dft_latest['open_interest']).sum()
    ...
    if not dft_with_hist_greeks.empty:
        ...
        if not df_recent_options.empty and pd.notna(spot_price):
    ...
    if not dft_with_hist_greeks.empty and not df_spot_hist.empty:
        example_instruments = [instr for instr in dft_latest.nlargest(3, 'open_interest')['instrument_name']] if not dft_latest.empty else dft_with_hist_greeks['instrument_name'].unique()[:3]
        for instr_name in example_instruments:
            ...
            if len(df_beta_data) > 12:
                beta_ts = calculate_option_beta(df_beta_data['mark_price_close'], df_beta_data['close'], rolling_window=12)
            ...
            if len(df_flow_input) > 20:
                ...
                if spot_adjusted_flow.notna().any():
                    fig_spot_adj_flow = go.Figure()
                    fig_spot_adj_flow.add_trace(go.Scatter(x=df_flow_input['date_time'].iloc[1:], y=spot_adjusted_flow.cumsum().iloc[1:], mode='lines'))
    ...
    if not dft_with_hist_greeks.empty and pd.notna(spot_price) and e_str:
    ...
    if st.sidebar.checkbox("Show MM D-G Hedge Sim Plot", value=True):
        ...
        if not dft_with_hist_greeks.empty and not df_krak_5m.empty and selected_expiry and not dft_latest.empty:
            ...
            if gamma_hedger_candidate_df.empty and not dft_latest[dft_latest['option_type'] == 'C'].empty:
                gamma_hedger_candidate_df = dft_latest[dft_latest['option_type'] == 'C'].loc[[abs(dft_latest[dft_latest['option_type'] == 'C']['k'] - spot_price).idxmin()]]
            if not gamma_hedger_candidate_df.empty:
                ...
                with Pool(processes=4) as pool:
                    mm_dg_portfolio_state_df_sim, mm_dg_hedge_actions_df_sim = pool.apply(mm_dg_sim_instance.run_loop, (min(5, pair_sim_lookback_days),))
